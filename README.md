...
### [Live Demo (시연 주소)](https://readvoice-nine.vercel.app/)
...
- **Smart Noise Cancellation (지능형 노이즈 필터링)**:
  ...
  - **AI 지능형 정제**: Llama 3.3의 텍스트 정제 모드를 통해 '음', '어' 등의 간투사와 문맥 이탈 노이즈를 지능적으로 제거.
...
#### [Prompt Example: 전문 회의록 생성]

> AI가 정해진 포맷을 벗어나지 않도록 페르소나와 제약 사항을 명확히 정의했습니다.

```text
당신은 전문적인 비서입니다. 음성 인식(STT) 과정에서 발생한 노이즈는 무시하고 핵심 내용만 추출하세요.
1. 절대 한자(漢字)를 사용하지 마세요. 모든 단어는 순수 한글로만 작성해야 합니다.
2. 반드시 다음의 JSON 형식으로만 응답하세요:
{
  "topic": "회의 주제 요약",
  "discussions": ["주요 논의 사항 1", ...],
