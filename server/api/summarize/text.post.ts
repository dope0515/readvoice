...
반드시 다음 규칙을 지키세요:
1. **절대 한자(漢字)를 사용하지 마세요.** 모든 단어는 반드시 순수 한글로만 작성해야 합니다. (예: 比較 -> 비교, 發展 -> 발전)
...

{
  ...
  "actionItems": ["추후 진행 및 확인 필요 사항 1"]
    ...
    systemPrompt = '당신은 텍스트 요약 전문가입니다. 음성 인식 오류로 인한 불필요한 영어, 특수 문자, 노이즈를 제거하고 핵심 내용만 3개 요점으로 요약해주세요. **절대 한자(漢字)를 사용하지 말고, 모든 내용을 100% 한글로만 작성하세요.**'
    userPrompt = `다음 텍스트에서 노이즈를 제거하고 3개 요점으로 요약해주세요:\n\n${text}`
  } else if (mode === 'format') {
    systemPrompt = `당신은 텍스트 노이즈 제거기입니다. 음성 인식(STT) 결과물에서 원래 발언자의 의도를 절대 훼손하지 말고 오직 아래 언급된 '불필요한 노이즈'만 제거하세요:
1. 문맥과 상관없는 무의미한 영어 단어, 특수 문자, 외국어 노이즈(예: matte, avis 등)는 삭제하세요.
2. 텍스트에 포함된 문맥에 맞지 않는 한자(漢字)는 한국어(한글)로 변환하거나 삭제하세요.
3. '음', '어', '그', '아' 같은 무의미한 간투사나 단순 반복되는 소리만 제거하세요.
4. **[매우 중요] 절대 문장을 새롭게 지어내거나(Paraphrasing), 단어를 유의어로 바꾸거나, 어순을 변경하지 마세요. 원래 내용을 100% 그대로 유지해야 합니다.**
5. 원래 끊겨있는 문장이면 끊긴 대로, 어색한 문장이면 어색한 대로 원문을 보존하세요. 자연스럽게 만들려고 시도하지 마세요.
6. 원문의 노이즈만 제거된 텍스트만 출력하고 설명이나 주석은 절대 포함하지 마세요.`
    userPrompt = `다음 음성 변환 텍스트에서 원문을 그대로 유지하며 노이즈와 간투사만 제거해주세요:\n\n${text}`
