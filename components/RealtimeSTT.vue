      ...
      if (chunks.length > 0) {
        ...
        try {
          ...
          if (error.statusCode === 429 || error.message?.includes('Rate limit')) {
            errorMessage.value = 'API 사용량이 초고되었습니다. 잠시 후 다시 시도해주세요.'
          } else {
            errorMessage.value = '변환 중 오류가 발생했습니다: ' + (error.message || '알 수 없는 오류')
          }
        }
      }
    }
...
const handleDownloadAudio = () => {
  ...
  a.href = url
  a.download = `${getFormattedFilename('녹음음성')}.wav`
  a.click()
...
const handleExportPdf = async () => {
  ...
  try {
    await exportToPdf('meeting-minutes-area-realtime', getFormattedFilename())
  } catch (e) {
    alert('PDF 생성 실패')
  } finally {
